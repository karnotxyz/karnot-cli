...
impl DaConfig for AvailConfig {
    fn setup_and_generate_keypair(&self, app_chain: &str) -> Result<(), KeyGenError> {
        generate_avail_keypair(app_chain)?;

        Ok(())
    }
}

fn generate_avail_keypair(app_chain: &str) -> Result<(), KeyGenError> {
    generate_keypair(app_chain)?;

    Ok(())
}

fn generate_keypair(app_chain: &str) -> Result<(), KeyGenError> {
